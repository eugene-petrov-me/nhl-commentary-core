...
summarize_game_module = importlib.import_module("engine.summarize_game")
...
def test_summarize_game_rule_based(monkeypatch):
    ...
    monkeypatch.setattr(summarize_game_module, "process_game_events", fake_process_game_events)
    monkeypatch.setattr(summarize_game_module, "get_or_build_stats_summary", fake_get_or_build_stats_summary)
    monkeypatch.setattr(summarize_game_module, "generate_ai_summary", fake_generate_ai_summary)
...
def test_summarize_game_ai(monkeypatch):
    ...
    def fake_get_or_build_stats_summary(*args, **kwargs):
    ...
    monkeypatch.setattr(summarize_game_module, "get_or_build_stats_summary", fake_get_or_build_stats_summary)
    monkeypatch.setattr(summarize_game_module, "generate_ai_summary", fake_generate_ai_summary)
    monkeypatch.setattr(summarize_game_module, "get_play_by_play", lambda game_id: ["event"])
    monkeypatch.setattr(summarize_game_module, "get_game_story", lambda game_id: {"story": "data"})
